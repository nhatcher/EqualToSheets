...
describe("utils", () => {
  ...
  describe("convertSpreadsheetDateToDayjsUTC", () => {
    test("converts integer dates (full days)", () => {
      expect(convertSpreadsheetDateToDayjsUTC(0).toISOString()).toEqual(
  ...
  describe("convertDayjsUTCToSpreadsheetDate", () => {
    test("converts integer dates (full days)", () => {
      expect(
        convertDayjsUTCToSpreadsheetDate(dayjs.utc("1899-12-30T00:00:00.000Z"))
      ).toEqual(0);
      expect(
        convertDayjsUTCToSpreadsheetDate(dayjs.utc("1950-08-15T00:00:00.000Z"))
      ).toEqual(18_490);
      expect(
        convertDayjsUTCToSpreadsheetDate(dayjs.utc("2012-11-21T00:00:00.000Z"))
      ).toEqual(41_234);
      expect(
        convertDayjsUTCToSpreadsheetDate(dayjs.utc("2030-01-01T00:00:00.000Z"))
      ).toEqual(47_484);
    });

    test("converts floating point dates (days with additional seconds)", () => {
      expect(
        convertDayjsUTCToSpreadsheetDate(dayjs.utc("1899-12-30T02:24:00.000Z"))
      ).toBeCloseTo(0.1);
      expect(
        convertDayjsUTCToSpreadsheetDate(dayjs.utc("1950-08-15T04:48:00.001Z"))
      ).toBeCloseTo(18_490.2);
      expect(
        convertDayjsUTCToSpreadsheetDate(dayjs.utc("2012-11-21T07:12:00.000Z"))
      ).toBeCloseTo(41_234.3);
      expect(
        convertDayjsUTCToSpreadsheetDate(dayjs.utc("2030-01-01T09:36:00.000Z"))
      ).toBeCloseTo(47_484.4);
    });
  });
});
