...
use equalto_xlsx::import::load_model_from_xlsx_without_support_check;
use std::io;
use std::sync::mpsc;
use std::thread;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut model = if args.len() > 1 {
        ...
        load_model_from_xlsx_without_support_check(file_name, "en", "UTC").unwrap()
    } else {
    ...
    loop {
        terminal.draw(|rect| {
            ...
            let sheet_list = Table::new(rows)
                .block(sheets)
                .widths(&[Constraint::Length(100)])
                .column_spacing(0);
            ...
            let spreadsheet = Table::new(rows)
            ...
            let formula_bar = Paragraph::new(vec![Spans::from(vec![Span::raw(formula_bar_text)])]);
            ...
            CursorMode::Navigate => {
                match rx.recv()? {
                    Event::Input(event) => match event.code {
                        ...
                        KeyCode::Char('s') => {
                            if selected_sheet < sheet_names.len() - 1 {
                                selected_sheet += 1;
                            } else {
                                selected_sheet = 0;
                            }
            ...
            CursorMode::Input => match rx.recv()? {
                Event::Input(event) => match event.code {
                    ...
                    KeyCode::Enter => {
                        ...
                        // let style_index = model.get_cell_style_index(sheet as u32, row, column);
