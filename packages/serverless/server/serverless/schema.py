...
def get_license(info) -> models.License:
...
def _validate_license_for_workbook(info, workbook_id):
...
def validate_license_for_workbook_mutation(graphql_fn):
    def fn(cls, root, info, workbook_id, *args, **kwargs):
        _validate_license_for_workbook(info, workbook_id)
        return graphql_fn(cls, root, info, workbook_id, *args, **kwargs)
    return fn

def validate_license_for_workbook_query(graphql_fn):
    def fn(root, info, workbook_id, *args, **kwargs):
        _validate_license_for_workbook(info, workbook_id)
...
class Sheet(graphene.ObjectType):
    ...
    def resolve_cell(root, info, ref=None, row=None, col=None):
...
class Workbook(DjangoObjectType):
    ...
    def resolve_sheet(root, info, sheet_id=None, name=None):
...
class Query(graphene.ObjectType):
    ...
    def resolve_workbook(root, info, workbook_id):
        return models.Workbook.objects.get(id=workbook_id)

    def resolve_workbooks(root, info):
        license = get_license(info)
...
class SaveWorkbook(graphene.Mutation):
    ...
    def mutate(cls, root, info, workbook_id, workbook_json):
...
class SetCellInput(graphene.Mutation):
    ...
    @classmethod
    @validate_license_for_workbook_mutation
    def mutate(cls, root, info, workbook_id, input, sheet_id=None, sheet_name=None, ref=None, row=None, col=None):
        ...
        assert (ref is None and row is not None and col is not None) or (ref is not None and row is None and col is None)
        workbook = models.Workbook.objects.get(id=workbook_id)
...
class CreateWorkbook(graphene.Mutation):
    ...
    def mutate(cls, root, info):
        ...
        license = get_license(info)
        origin = info.context.META.get('HTTP_ORIGIN')
        log.info('CreateWorkbook: auth=%s, license=%s, origin=%s'%(info.context.META.get('HTTP_AUTHORIZATION'), license, origin))
        if not is_license_key_valid_for_host(license.key, origin):
